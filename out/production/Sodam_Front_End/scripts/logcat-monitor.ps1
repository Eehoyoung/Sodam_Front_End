...
if ($Append) { Add-Content -LiteralPath $Out -Value $sessionHeader }
else { Set-Content -LiteralPath $Out -Value $sessionHeader }
...
try {
  while (-not $proc.HasExited) {
    if ($TimeoutSec -gt 0) {
      ...
      if ($elapsed.TotalSeconds -ge $TimeoutSec) { break }
    }
