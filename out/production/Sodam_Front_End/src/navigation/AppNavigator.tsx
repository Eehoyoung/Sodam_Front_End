 ...
 */
type AppNavigatorProps = { appReady?: boolean };
const AppNavigator: React.FC<AppNavigatorProps> = ({appReady = true}) => {
    ...
    const getInitialRoute = () => {
        ...
        // Unauthenticated users land on Welcome (HybridMainScreen)
        return "Welcome";
    };
    ...
    useEffect(() => {
        // Gate any automatic navigation or modal actions until both flags are ready
        // Example:
        // if (navReady && appReady) {
        //   // safe to navigate or open modal
        // }
    }, [navReady, appReady]);

    // Until JS is fully ready (post interactions), render a lightweight splash to avoid early focus work
    if (!appReady) {
        return (
            <View style={styles.loadingContainer}>
                <ActivityIndicator size="large" color="#2196F3"/>
